 ...
 * @author Aluno
 */
@WebServlet(name = "PublicoCarregar", urlPatterns = {"/PublicoCarregar"})
public class PublicoCarregar extends HttpServlet {

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setContentType("text/html;charset=iso-8859-1");
        int idEstado = Integer.parseInt(request.getParameter("idPublico"));
        try {
            GenericDAO dao = new PublicoDao();
            request.setAttribute("publico", dao.carregar(idEstado));
